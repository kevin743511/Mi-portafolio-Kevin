...
async function handleFiles(fileList) {
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    try {
      ...
      console.error('❌ Exception subiendo archivo:', e);
      soundError();
      showToast('✕ Error inesperado: ' + file.name, 'te');
    }
  }
...
async function saveEdit() {
  ...
  try {
    ...
    if (error) {
      ...
      soundError();
      showToast('✕ Error actualizando archivo: ' + error.message, 'te');
    } else {
    ...
    console.error('❌ Exception actualizando:', e);
    soundError();
    showToast('✕ Error de conexión', 'te');
  }
}
...
async function deleteFile(id) {
  ...
  try {
    ...
    if (dbErr) {
      ...
      soundError();
      showToast('✕ Error eliminando archivo: ' + dbErr.message, 'te');
    } else {
    ...
    console.error('❌ Exception eliminando:', e);
    soundError();
    showToast('✕ Error de conexión', 'te');
  }
}
...
async function init() {
  ...
  try {
    ...
    console.error('❌ Exception cargando archivos públicos:', e);
  }
